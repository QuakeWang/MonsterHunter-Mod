...
function fps_handler(retval)
	if config.enableFPS then
		if config.autoFPS then
			local FrameRateOption = sdk.get_managed_singleton("snow.StmOptionManager"):get_field(
			"_StmOptionDataContainer"):call("getFrameRateOption")
...
sdk.hook(sdk.find_type_definition("snow.StmOptionManager"):get_method("writeGraphicOptionOnIniFile"), pre_hook,
	fps_handler)                                                                                                             -- allows title screen fps changes to appear immediately, if there's a better method to hook, let me know
sdk.hook(sdk.find_type_definition("snow.eventcut.UniqueEventManager"):get_method("playEventCommon"), pre_hook,
	fps_handler)                                                                                                             -- only bother setting fps for cutscenes
...
if modUI then
	modObj = modUI.OnMenu("RiseTweaks",
		"Unlock the framerate cap in cutscenes.\nCan also set your image quality higher than 150%.", function()
		local changed = false
		...
		if config.enableFPS then
			changed, config.autoFPS = modUI.CheckBox("Automatic Frame Rate", config.autoFPS,
				"Automatically determine the framerate used in cutscenes.\nThis feature pulls from the game's options.\n(Options>Display>Framerate Cap)")
			if not config.autoFPS then
				changed, config.desiredFPS = modUI.Slider("Frame Rate", config.desiredFPS, 10, 600,
					"Set the framerate to be used in cutscenes.")
			end
		end
		if changed then
			fps_handler()
		...
		changed, config.enableQuality = modUI.CheckBox("Enable", config.enableQuality,
			"Use a custom value instead of the game's Image Quality.\n(Options>Display>Advanced Graphics Settings)")
		if config.enableQuality then
			changed, config.desiredQuality = modUI.FloatSlider("Image Quality", config.desiredQuality, 0.05, 4.0,
				"Set the Image Quality to be used in cutscenes.")
		end
		if changed then
			quality_handler()
		end
